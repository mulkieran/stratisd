...
impl Backstore {
    ...
    pub fn reencrypt(
        ...
        assert!(blockdevs.len() == key_info.len());

        for (bd, (slot, key, new_slot)) in blockdevs.into_iter().map(|(_, _, bd)| bd).zip(key_info)
        {
            bd.do_reencrypt(pool_uuid, slot, key, new_slot)?;
        }
        Ok(())
    }
    ...
    pub fn remove_cache(
        ...
        if let Err(e) = resume_res {
            ...
            self.linear = None;
            return Err(StratisError::Chained(
                "Failed to load the pending table for the thin pool".to_string(),
                Box::new(e),
            ));
        }
